...
describe('bls12-381 Point', () => {
  describe('Point with Fq coordinates', () => {
    ...
    it('should be placed on curve vector 2', () => {
      const a = new PointG1(CURVE,
        new Fq(CURVE,
          bigInt('17f1d3a73197d7942695638c4fa9ac0fc3688c4f9774b905a14e3a3f171bac586c55e83ff97a1aeffb3af00adb22c6bb', 16)
        ),
        new Fq(CURVE,
    ...
    it('should not be placed on curve vector 1', () => {
      ...
      expect(() => a.assertValidity()).to.throw();
    });
    ...
    it('should not be placed on curve vector 3', () => {
      const a = new PointG1(CURVE,
        new Fq(CURVE,
          bigInt('034a6fce17d489676fb0a38892584cb4720682fe47c6dc2e058811e7ba4454300c078d0d7d8a147a294b8758ef846cca', 16)
        ),
        new Fq(CURVE,
          bigInt('14e4b429606d02bc3c604c0410e5fc01d6093a00bb3e2bc9395952af0b6a0dbd599a8782a1bea48a2aa4d8e1b1df7caa', 16)
        ),
        new Fq(CURVE,
          bigInt('1167e903c75541e3413c61dae83b15c9f9ebc12baba015ec01b63196580967dba0798e89451115c8195446528d8bcfca', 16)
        ),
      );
      expect(() => a.assertValidity()).throw();
    });
  ...
  describe('Point with Fq2 coordinates', () => {
    ...
    it('should not be placed on curve vector 3', () => {
      const a = new PointG2(CURVE,
        Fq2.fromTuple(CURVE, [
          bigInt('877d52dd65245f8908a03288adcd396f489ef87ae23fe110c5aa48bc208fbd1a0ed403df5b1ac137922b915f1f38ec37', 16),
          bigInt('0cf8158b9e689553d58194f79863fe02902c5f169f0d4ddf46e23f15bb4f24304a8e26f1e5febc57b750d1c3dc4261d8', 16)
        ]),
        Fq2.fromTuple(CURVE, [
          bigInt('065ae9215806e8a55fd2d9ec4af9d2d448599cdb85d9080b2c9b4766434c33d103730c92c30a69d0602a8804c2a7c65f', 16),
          bigInt('0e9c342d8a6d4b3a1cbd02c7bdc0e0aa304de41a04569ae33184419e66bbc0271c361c973962955ba6405f0e51beb98b', 16)
        ]),
        Fq2.fromTuple(CURVE, [
          bigInt('19cbaa4ee4fadc2319939b8db45c6a355bfb3755197ba74eda8534d2a2c1a2592475939877594513c326a90c11705002', 16),
          bigInt('0c0d89405d4e69986559a56057851733967c50fd0b4ec75e4ce92556ae5d33567e6e1a4eb9d83b4355520ebfe0bef37c', 16)
        ]),
      );
      expect(() => a.assertValidity()).throw();
    });
